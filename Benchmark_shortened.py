...
from CV_helper import ClickVersion
...
test_server = r"C:/Fabric1.19.4-BenchmarkServer"

# Java Executable Path

java_path = r"C:/Benchmark/JDKs/GraalVM/bin/java.exe"

# Java Flags (for servers)
# (Should start with a space, so they can be "added" together with the + sign)

# Legacy server flag collections from Aikar and BrucetheMoose which can be used to instantiate a flags attribute
...
# add to flags if user has large pages enabled
lpages = r""" -XX:+UseLargePages -XX:LargePageSizeInBytes=2m"""

# memory options
number_of_gigs = 10
memory = f" -Xms{number_of_gigs}G -Xmx{number_of_gigs}G"

# -----------------------Benchmark Data--------------------------
benchname = r"Development Testing"  # Name for the whole benchmark run
...
command = java_path + memory + flags
has_lpages = False
if has_lpages:
    command += lpages
...
blist = [
    ...
    #    "ClientLauncher": "Path to Minecraft Windows Launcher folder",
    ...
    # ]
    {
        ...
        "LauncherPath": "some/windows/path",
        "Iterations": 3,
    },
    {
        "Name": "Test Server Benchmark",
        "Command": java_path + memory + graalflags,
        "Path": test_server,
        "Iterations": 5,
    },
]
...
# Server benchmarking class options
...
fabric_chunkgen_command = (
    r"chunky start"  # Chunk generation command to use in fabric packs
)
fabric_chunkgen_expect = r"[Chunky] Task finished for"  # String to look for when chunk generation is finished
forge_chunkgen_command = (
    r"forge generate 0 0 0 3000"  # Chunk generation command to use in forge packs
)
forge_chunkgen_expect = (
    r"Finished generating"  ##String to look for when chunk generation is finished
)
startuptimeout = (
    350  # Number of seconds to wait before considering the server to be dead/stuck
)
...
# Client benchmarking class options
...
presentmonpath = (
    r"C:/Benchmark/presentmon.exe"  # full path to Intel presentmon executable file
)
...
def benchmark(i):  # "i is the benchmark index"
    ...
    if "PrismInstance" in blist[i] and ("Command" in blist[i] or "Path" in blist[i]):
        raise Exception(
            "Each benchmark instance should ether have a command and path entry, or a Prism instance entry, not both"
        )
    ...
    if "PrismInstance" in blist[i]:
        ...
        prismfolder = os.path.normpath(
            os.path.join(
                os.path.dirname(prismpath), "instances", blist[i]["PrismInstance"]
            )
        )
        ...
        prismfolder = (
            glob.glob(os.path.join(prismfolder, "minecraft"))
            + glob.glob(os.path.join(prismfolder, ".minecraft"))
        )[0]
        if not os.path.isdir(prismfolder):
        ...
        restore_world(
            worldfolder, worldbackup
        )  # restore backup in case it wasnt restored on exit before

        for n in range(
            1, blist[i]["Iterations"] + 1
        ):  # Run benchmark for # of iterations
            try:
                backup_world(worldfolder, worldbackup)

                for proc in psutil.process_iter(
                    ["name"]
                ):  # Check for an existing javaw process
                ...
                try:
                    clientprocess = subprocess.Popen(
                        [prismpath, "--launch", blist[i]["PrismInstance"]],
                ...
                blist[i]["Average_FPS"].append(
                    round(1000 / statistics.mean(frametimes), 2)
                )  # Average FPS
                blist[i][r"1%_Frametime_ms"].append(
                    round(
                        statistics.mean(
                ...
                blist[i][r"5%_Frametime_ms"].append(
                    round(
                        statistics.mean(
                ...
                time.sleep(
                    9
                )  # Give the client some time to close, otherwise it may not start up again.
                restore_world()
                with open(benchlog, "w") as f:
                    json.dump(
                        blist[0 : i + 1], f, indent=4
        ...
        try:
            if blist[i]["Iterations"] >= 2:
                ...
                blist[i][r"Average_1%_Frametime_ms"] = safemean(
                    blist[i][r"1%_Frametime_ms"]
                )
                blist[i][r"PVariance_1%_Frametime_ms"] = safevar(
                    blist[i][r"1%_Frametime_ms"]
                )
                blist[i][r"Average_5%_Frametime_ms"] = safemean(
                    blist[i][r"5%_Frametime_ms"]
                )
                blist[i][r"PVariance_5%_Frametime_ms"] = safevar(
                    blist[i][r"5%_Frametime_ms"]
                )
                if spark:
        ...
        def restore_world():
            if os.path.isdir("world") and os.path.isdir("_world_backup"):
                try:
                    ...
                    time.sleep(
                        7
                    )  # The old server is still up, give it some time to close
                    shutil.rmtree("world")
            ...
            try:
                ...
                try:
                    ...
                    mcserver = popen_spawn.PopenSpawn(
                        command, timeout=totaltimeout, maxread=20000000
                ...
                if plat == "Windows":
                    try:
                        for proc in psutil.process_iter(
                            ["name"]
                        ):  # Set to high process priority in windows, for greater consistency when run in the background
                ...
                if not crash:
                    ...
                    if hascarpet:
                        ...
                        blist[i]["Player_Spawn_Times"].append(
                            ...
                            with open(
                                ...
                                for l in lines:
                                    if "TPS from last 5" in l:
                                        blist[i]["Average_TPS_Values"].append(
                                            float(
                                                lines[iter + 1]
                                                .split(",")[-1][1:-1]
                                                .split("*")[-1]
        ...
        try:  # Dont let funky data kill the benchmark
            if blist[i]["Iterations"] >= 2:
                ...
                blist[i]["PVariance_Chunkgen_Time"] = safevar(
                    blist[i]["Chunkgen_Times"]
                )
                ...
                if carpet:
                    blist[i]["Average_Spawn_Time"] = safemean(
                        blist[i]["Player_Spawn_Times"]
                    )
                    blist[i]["Player_Spawn_Variance"] = safevar(
                        blist[i]["Player_Spawn_Times"]
                    )
        except Exception as e:
